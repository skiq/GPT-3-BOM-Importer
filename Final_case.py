...
def Upload_File_for_gpt3(text_file):
    ...
    with jsonlines.open(r'UploadedFile/format_training_data.jsonl', 'w') as writer:
...
def run_gpt(ques, example_text, q_a_examples, file_id):
...
def get_MPN_and_index(example_text, q_a_examples):
    ...
    for it in (ques_and_index):
        ques = it[0]
        MPN = run_gpt(ques, example_text, q_a_examples, file_id)
        index = it[1]
...
#Step 8 getting the MPN and index to be sent to validator
